...
export function getInstalledInjectedConnectors(): InjectedConnector[] {
  ...
  return shuffled.map((id) => new InjectedConnector({ id }))
}
...
function getInstalledWallets(): Record<string, IStarknetWindowObject> {
  ...
  return Object.getOwnPropertyNames(window).reduce<Record<string, IStarknetWindowObject>>((wallets, key) => {
...
const shuffle = <T extends Array<unknown>>(arr: T): T => {
